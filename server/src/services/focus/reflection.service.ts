...
const FALLBACK_JOURNAL: JournalRecord = {
...
export async function generateJournal(
  ...
  if (!apiKey) return FALLBACK_JOURNAL;
...
Next steps: ${reflection.nextSteps}`;
  ...
  try {
    ...
    const parsed = JSON.parse(rawText) as Partial<JournalRecord>;
    ...
    return {
      ...
      encouragement:
        typeof parsed.encouragement === 'string' ? parsed.encouragement : FALLBACK_JOURNAL.encouragement,
      createdAt: new Date().toISOString(),
    };
  } catch {
