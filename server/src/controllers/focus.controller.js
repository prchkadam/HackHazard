...
import { startFocusSession, finishFocusSession, submitReflectionAndGenerateJournal, getFocusHistory, } from '../services/focus/focus.service';
// ──────────────────────────────
// POST /focus/session
// ──────────────────────────────
...
export async function updateSession(req, res) {
    ...
    if (!sessionId) {
        sendError(res, 'Session ID required', 'VALIDATION_ERROR', 400);
        return;
    }
    try {
...
export async function submitReflection(req, res) {
    ...
    try {
        const result = await submitReflectionAndGenerateJournal(sessionId, req.user.userId, parsed.data.topic, { learned: parsed.data.learned, difficult: parsed.data.difficult, nextSteps: parsed.data.nextSteps });
        sendSuccess(res, result);
    }
    catch {
